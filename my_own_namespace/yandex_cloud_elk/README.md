...
hosts: localhost
gather_facts: false
roles:role: my_own_namespace.yandex_cloud_elk.create_file
vars:
...
hosts: localhost
gather_facts: false
tasks:name: Create file using custom module
...
ТипОбязательныйОписаниеpathstringДаАбсолютный путь к целевому файлу.contentstringДаТекстовое содержимое для записи.
